#Do not code any more past this, can figure out making the collision not count so much during interview, look it up, but don't look to much into it, now practice pairing and communication
...
score = 0 #this has to be outside the loop so that it doesn't constantly re-set to zero
...
while run: #this is the loop of the actual game
    ...
    if paul_rect.colliderect(feyd1_rect) or paul_rect.colliderect(feyd2_rect) or paul_rect.colliderect(feyd3_rect) or paul_rect.colliderect(feyd4_rect):
        score += 1 
